...
function QuizScreen() {
  const { section3, dispatch } = useQuiz();

  const numQuestions = section3.questions.length;
  const totalPoints = section3.questions.reduce(
    ...
    <>
      ...
      <div className="flex justify-between mb-10">
        <div className=" text-2xl font-semibold">
          ...
          <span className="mb-4 text-2xl font-semibold ">Section 3</span>
        </div>
        {section3.index ? (
          <button
            // onClick={handleShowPoints}
            onClick={() =>
              dispatch({ type: 'show_points', section: 'section3' })
            }
      ...
      {section3.index > numQuestions - 1 ? (
        <ScoreBoard points={section3.points} totalPoints={totalPoints} />
      ) : (
        <div>
            ...
            <div className="flex-1 mb-4">
              {section3.index <= numQuestions - 1 && (
                <p className="text-2xl mb-2">
                  Question <strong>{section3.index + 1}</strong> of{' '}
                  {numQuestions}
                </p>
              )}
              <p className=" text-2xl mb-2 font-semibold">
                {section3.questions[section3.index]?.question}
              </p>
              {section3.questions[section3.index]?.options.map((option, i) => (
                <button
                  key={option}
          ...
          <div className="flex justify-between">
            {section3.answer !== null ? (
              <p className="mt-4">
                {section3.index < numQuestions - 1 ? (
                  <button
                    // onClick={handleNext}
                    onClick={() =>
                      dispatch({ type: 'next_question', section: 'section3' })
                    }
